...
#ifndef REGPRO_POD_VECTOR_H
#define REGPRO_POD_VECTOR_H

#include <cstddef>
#include <cstdint>
#include <string.h>
#include "pod_allocator.h"

namespace pod {

//-------------------------------------------------------array_adaptor
template<class T> class array_adaptor
{
public:
    typedef T value_type;
    array_adaptor(T* array, unsigned size) : 
...
//---------------------------------------------------------auto_array
template<class T, unsigned Size> class auto_array
{
public:
    typedef T value_type;
    typedef auto_array<T, Size> self_type;

    auto_array() {}
    explicit auto_array(const T* c)
    {
...
//--------------------------------------------------------auto_vector
template<class T, unsigned Size> class auto_vector
{
public:
    typedef T value_type;
    typedef auto_vector<T, Size> self_type;

    auto_vector() : m_size(0) {}
...
public:
    ...
    ~array() { allocator<T>::deallocate(m_array, m_size); }
    array() : m_array(0), m_size(0) {}

    array(unsigned size) : 
        m_array(allocator<T>::allocate(size)), 
        m_size(size) 
    {}

    array(const self_type& v) : 
        m_array(allocator<T>::allocate(v.m_size)), 
        m_size(v.m_size) 
    {
    ...
    void resize(unsigned size)
    {
        if(size != m_size)
        {
            allocator<T>::deallocate(m_array, m_size);
            m_array = allocator<T>::allocate(m_size = size);
        }
    }
...
public:
    ...
    ~vector() { allocator<T>::deallocate(m_array, m_capacity); }
    vector() : m_size(0), m_capacity(0), m_array(0) {}
    vector(unsigned cap, unsigned extra_tail=0);
...
//------------------------------------------------------------------------
template<class T> 
void vector<T>::capacity(unsigned cap, unsigned extra_tail)
{
    m_size = 0;
    if(cap > m_capacity)
    {
        allocator<T>::deallocate(m_array, m_capacity);
        m_capacity = cap + extra_tail;
        m_array = m_capacity ? allocator<T>::allocate(m_capacity) : 0;
    }
}

//------------------------------------------------------------------------
template<class T> 
void vector<T>::allocate(unsigned size, unsigned extra_tail)
{
...
//------------------------------------------------------------------------
template<class T> 
void vector<T>::resize(unsigned new_size)
{
    if(new_size > m_size)
    {
        if(new_size > m_capacity)
        {
            T* data = allocator<T>::allocate(new_size);
...
//------------------------------------------------------------------------
template<class T> vector<T>::vector(unsigned cap, unsigned extra_tail) :
    m_size(0), 
    m_capacity(cap + extra_tail), 
    m_array(allocator<T>::allocate(m_capacity)) {}

//------------------------------------------------------------------------
template<class T> vector<T>::vector(const vector<T>& v) :
    m_size(v.m_size),
    m_capacity(v.m_capacity),
    m_array(v.m_capacity ? allocator<T>::allocate(v.m_capacity) : 0)
{
...
//------------------------------------------------------------------------
template<class T> void vector<T>::serialize(uint8_t* ptr) const
{ 
...
//------------------------------------------------------------------------
template<class T> 
void vector<T>::deserialize(const uint8_t* data, unsigned byte_size)
{
    byte_size /= sizeof(T);
    allocate(byte_size);
...
//------------------------------------------------------------------------
template<class T> 
void vector<T>::insert_at(unsigned pos, const T& val)
{
    if(pos >= m_size) 
    {
        m_array[m_size] = val;
...
//------------------------------------------------------------------------
template<class T, unsigned S=6> class bvector
{
public:
    enum block_scale_e
    {   
    ...
    ~bvector();
    bvector();
    bvector(unsigned block_ptr_inc);

    // Copying
    bvector(const bvector<T, S>& v);
    const bvector<T, S>& operator = (const bvector<T, S>& v);
...
//------------------------------------------------------------------------
template<class T, unsigned S> bvector<T, S>::~bvector()
{
    if(m_num_blocks)
    {
        ...
        while(m_num_blocks--)
        {
            allocator<T>::deallocate(*blk, block_size);
            --blk;
        }
    }
    allocator<T*>::deallocate(m_blocks, m_max_blocks);
}


//------------------------------------------------------------------------
template<class T, unsigned S> 
void bvector<T, S>::free_tail(unsigned size)
{
    if(size < m_size)
    {
        ...
        {
            allocator<T>::deallocate(m_blocks[--m_num_blocks], block_size);
        }
        if(m_num_blocks == 0)
        {
            allocator<T*>::deallocate(m_blocks, m_max_blocks);
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
bvector<T, S>::bvector(unsigned block_ptr_inc) :
    m_size(0),
    m_num_blocks(0),
    m_max_blocks(0),
    m_blocks(0),
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
bvector<T, S>::bvector(const bvector<T, S>& v) :
    m_size(v.m_size),
    m_num_blocks(v.m_num_blocks),
    m_max_blocks(v.m_max_blocks),
    m_blocks(v.m_max_blocks ? 
             allocator<T*>::allocate(v.m_max_blocks) : 
             0),
    m_block_ptr_inc(v.m_block_ptr_inc)
{
    ...
    {
        m_blocks[i] = allocator<T>::allocate(block_size);
...
template<class T, unsigned S> 
const bvector<T, S>& 
bvector<T, S>::operator = (const bvector<T, S>& v)
{
    unsigned i;
...
//------------------------------------------------------------------------
template<class T, unsigned S>
void bvector<T, S>::allocate_block(unsigned nb)
{
    if(nb >= m_max_blocks) 
    {
        T** new_blocks = allocator<T*>::allocate(m_max_blocks + m_block_ptr_inc);
    ...
    m_blocks[nb] = allocator<T>::allocate(block_size);
    m_num_blocks++;
}
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
inline void bvector<T, S>::add(const T& val)
{
    *data_ptr() = val;
    ++m_size;
}
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
inline void bvector<T, S>::remove_last()
{
    if(m_size) --m_size;
}
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
void bvector<T, S>::modify_last(const T& val)
{
    remove_last();
    add(val);
}
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
int bvector<T, S>::allocate_continuous_block(unsigned num_elements)
{
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
unsigned bvector<T, S>::byte_size() const
{
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
void bvector<T, S>::serialize(uint8_t* ptr) const
{
    unsigned i;
    for(i = 0; i < m_size; i++)
    {
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
void bvector<T, S>::deserialize(const uint8_t* data, unsigned byte_size)
{
    remove_all();
    byte_size /= sizeof(T);
...
//------------------------------------------------------------------------
template<class T, unsigned S> 
void bvector<T, S>::deserialize(unsigned start, const T& empty_val, 
                                    const uint8_t* data, unsigned byte_size)
{
    while(m_size < start)
    {
...
public:
    void remove_all()
    {
        if(m_num_blocks)
        {
            ...
            while(m_num_blocks--)
            {
                allocator<uint8_t>::deallocate(blk->data, blk->size);
                --blk;
            }
            allocator<block_type>::deallocate(m_blocks, m_max_blocks);
        }
        m_num_blocks = 0;
        m_max_blocks = 0;
        m_blocks = 0;
    ...
    void allocate_block(unsigned size)
    {
        ...
        if(m_num_blocks >= m_max_blocks) 
        {
            block_type* new_blocks = 
                allocator<block_type>::allocate(m_max_blocks + m_block_ptr_inc);
        ...
        m_blocks[m_num_blocks].data = 
            m_buf_ptr =
            allocator<uint8_t>::allocate(size);
