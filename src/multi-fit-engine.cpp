...
{
    struct multi_fit_engine *fit = (struct multi_fit_engine *) params;
...
{
    return multifit_fdf(x, params, f, nullptr);
}

static int
multifit_df(const gsl_vector *x, void *params, gsl_matrix *jacob)
{
    return multifit_fdf(x, params, nullptr, jacob);
}
...
multi_fit_engine_prepare(struct multi_fit_engine *fit)
{
    ...
    assert(fit->spectra_list != nullptr);
...
multi_fit_engine_new(struct fit_config const *cfg, int samples_number) {
    ...
    f = (struct multi_fit_engine *) emalloc(sizeof(struct multi_fit_engine));

    f->samples_number = samples_number;
    f->stack_list = (struct stack **) emalloc(samples_number * sizeof(void *));
    f->spectra_list = (struct spectrum **) emalloc(samples_number * sizeof(void *));
    f->acquisitions  = (struct acquisition_parameters *) emalloc(samples_number * sizeof(struct acquisition_parameters));

    memcpy(&f->config, cfg, sizeof(struct fit_config));

    /* fit parameters are provided later */
    f->common_parameters = nullptr;
    f->private_parameters = nullptr;
