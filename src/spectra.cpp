...
load_vase_spectrum(const char *filename, str_ptr *error_msg)
{
    ...
    if(data_table == nullptr) goto invalid_vase;
...
spectra_copy(struct spectrum *src) {
    struct spectrum *copy = (struct spectrum *) emalloc(sizeof(struct spectrum));
...
spectra_alloc(struct spectrum *s) {
    struct spectrum *synth = (struct spectrum *) emalloc(sizeof(struct spectrum));
