...
cmpl kramers_n_value_deriv(const disp_t *d, double lambda, cmpl_vector *pd) {
    ...
    for (int osc_index = KRAMERS_OSC_OFFS, k = 0; k < kramers->n;
        ...
        const auto x21 = x20 * (x10 * x12 + x17 * x4);
        const auto x22 = a * x20;
        const auto x23 = en * x16;
        const auto x24 = e * (-eg * x17 + x23);
        const auto x25 = (1.0 / 2.0) * x11;
        const auto x26 = en * x17;
        const auto x27 = 2 * x2;
        const auto x28 = -x1 + x27;
        const auto x29 = x0 * x28 / std::pow(x6, 2);
        const auto x30 = eg * x3;
        const auto x31 = 2 * x0;
        const auto x32 = x15 * x7;
        const auto x33 = x19 * x5;
        const auto x34 = x27 + x28 * x32;
        const auto x35 = en * x8;
        const auto x36 = a * x19;
        const auto x37 = x19 * (x14 - x16);
        const auto x38 = eg * x37 * x4;
        const auto x39 = x13 * x32;
        const auto x40 = x31 + x39;
        const auto x41 = x2 * x8;
        const auto x42 = (1.0 / 2.0) * x16;
        ...
        const double d_en_real =
            x36 * (x1 * x25 * x9 - x11 * x29 * x30 + x19 * x30 * x31 * x32 +
                   2 * x26 * x33 - x26 + x3 * x9 - x34 * x35 * x4);
        const double d_en_imag =
            x43 * (2 * en * x38 + x10 * x34 + x11 * x27 * x33 * x8 - x11 * x41 -
                   x15 * x2 * x29 + x4 * x41 + x42);
        const cmpl d_en = cmpl(d_en_real, -d_en_imag);
        pd->at(osc_index + KRAMERS_EN_OFFS) += d_en;

        const double d_eg_real =
            x1 * x36 * (-x11 * x18 * x19 * x35 + x25 * x35 + x38);
        const double d_eg_imag =
            x43 * (-eg * x1 * x19 * x23 + x13 - x18 * x37 - x42);
