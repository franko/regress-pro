...
struct disp_class bruggeman_disp_class = {
    DISP_BRUGGEMAN, "Bruggeman EMA", "bruggeman",
    bruggeman_free,
    bruggeman_copy,
    bruggeman_n_value,
    bruggeman_fp_number,
    bruggeman_n_value_deriv,
    bruggeman_apply_param,
    bruggeman_map_param,
    bruggeman_get_param_value,
    nullptr, nullptr,
    bruggeman_encode_param,
    bruggeman_read,
    bruggeman_write,
};
...
static cmpl epsilon_sqrt(const cmpl epsilon) {
    ...
    const double er = std::real(epsilon), ei = std::imag(epsilon);
...
bruggeman_copy(const disp_t *src)
{
    ...
    new_bema->components = (struct bema_component *) emalloc(src_bema->components_number * sizeof(struct bema_component));
...
bruggeman_new(const char *name, disp_t *disp_base, int components_number, struct bema_component *components)
{
    ...
    bema->components = (struct bema_component *) emalloc(components_number * sizeof(struct bema_component));
...
{
    return bruggeman_n_value_deriv(disp, lam, nullptr);
}
...
static cmpl epsilon_ode_solve(const epsilon_sequence *seq, const double ytol) {
    ...
    while (t < t_stop) {
        ...
        for (int k = 0; k < 20; k++) {
            ...
            if (fabs(std::real(state->yerr)) > ytol || fabs(std::imag(state->yerr)) > ytol) {
                t1 = t + (t1 - t) / 2;
                state->y = state->y0;
            } else {
                break;
...
static cmpl epsilon_newton_refine(const epsilon_sequence *seq, cmpl eps, const double eps_tol) {
    ...
    for (k = 0; k < 20; k++) {
        ...
        cmpl eps1 = eps + nsum / (3.0 * dsum);
        const cmpl delta_eps = eps1 - eps;
        eps = eps1;
        if (fabs(std::real(delta_eps)) < eps_tol && fabs(std::imag(delta_eps)) < eps_tol) {
            break;
        }
    }
    return eps;
...
int bruggeman_read(lexer_t *l, disp_t *d) {
    ...
    bema->disp_base = nullptr;
    bema->components_number = 0;
    bema->components = nullptr;
    int components_number;
    if (lexer_integer(l, &components_number)) return 1;
    bema->disp_base = disp_read(l);
    if (!bema->disp_base) return 1;
    struct bema_component *components = (struct bema_component *) emalloc(components_number * sizeof(struct bema_component));
    int i;
...
bruggeman_add_comp(disp_t *disp, int index, disp_t *new_comp) {
    ...
    struct bema_component *new_components = (struct bema_component *) malloc(sizeof(struct bema_component) * (n + 1));
