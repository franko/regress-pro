/* Generated with sympy using the command: tauc-lorentz-sympy.py
 * ../src/tauc-lorentz-template.cpp */
cmpl tauc_lorentz_n_value(const disp_t *d, double lambda) {
    ...
    for (int k = 0; k < nb; k++) {
        ...
        const auto x16 = ((x14) ? (std::log(-eg + en)) : (std::log(x15)));
        const auto x17 = 2 * eg;
        ...
        double eps1 =
            ((c < 1.9999999999 * e0)
                 ? (2 * x10 *
                    (x17 * x20 * x23 *
                         (x21 + 2 * std::atan(x20 * x22 * (2 * x2 + x8))) +
                     x20 * x29 * std::log((x25 + x28) / (x25 - x28)) -
                     x27 * (x21 + std::atan(x22 * (x17 - x19)) +
                            std::atan(x22 * (x17 + x19)))))
                 : (x18 *
                    (-c * x23 * x30 + x27 * std::atan((1.0 / 2.0) * c / eg) +
                     x29 * x30))) +
            ((en > eg - eg_delta && en < eg + eg_delta)
                 ? (-8 * en * x10 * x11 * std::log(4 * x0 / x6))
                 : (x11 * x18 *
                    (x12 * (x0 + x2) * (-x13 + x16) -
                     x17 * (x13 + x16 - std::log(x6)))));
        double eps2 = ((x14) ? (a * x11 * x12 * std::pow(x15, 2) /
                                (x0 * x1 + std::pow(x26, 2)))
                             : (0));
        er_sum += eps1;
        ei_sum += eps2;
