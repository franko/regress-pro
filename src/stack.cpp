...
void
stack_init_raw(stack_t *s, int nb_init)
{
    s->nb = 0;
    s->nb_alloc = nb_init;
    s->disp = (disp_t **) emalloc(nb_init * sizeof(void *));
    /* For thicknesses we allocate more space of what is actually needed. */
    s->thickness = (double *) emalloc(nb_init * sizeof(double));
}
...
{
    if(req_size > s->nb_alloc)
    {
        int nsz;
        s->nb_alloc *= 2;
        nsz = s->nb_alloc;
        s->disp = (disp_t **) erealloc(s->disp, nsz * sizeof(void *));
        /* For thicknesses we allocate more space of what is actually needed. */
        s->thickness = (double *) erealloc(s->thickness,  nsz * sizeof(double));
    }
}
...
{
    int j;
    stack_t *r = (stack_t *) emalloc(sizeof(stack_t));
    memcpy(r, s, sizeof(stack_t));

    r->nb_alloc = s->nb;

    r->thickness = (double *) emalloc(r->nb_alloc * sizeof(double));
    memcpy(r->thickness, s->thickness, r->nb_alloc * sizeof(double));

    r->disp = (disp_t **) emalloc(r->nb_alloc * sizeof(disp_t *));
    for(j = 0; j < r->nb; j++) {
...
{
    int i, nb;
    if (lexer_check_ident(l, "stack")) return nullptr;
    if (lexer_integer(l, &nb)) return nullptr;
