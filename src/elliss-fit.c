                       ...
                       double jacob_acq[], struct elliss_ab *result)
{
    ...
    case PID_LAYER_N:
        ...
        result->alpha = creal(drdn_alpha)*dnr - cimag(drdn_alpha)*dni;
        result->beta  = creal(drdn_beta) *dnr - cimag(drdn_beta) *dni;
        break;
    case PID_AOI:
               ...
               gsl_matrix * jacob)
{
    ...
    cmpl *jacob_n = (jacob && !fit->run->th_only ? jacob_n_data : NULL);
    double *jacob_acq = (jacob && fit->run->require_acquisition_jacob ? jacob_acq_data : NULL);
