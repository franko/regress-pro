...
struct disp_class disp_lookup_class = {
    DISP_LOOKUP, "Table Lookup", "lookup",
    lookup_free,
    lookup_copy,
    lookup_n_value,
    lookup_fp_number,
    lookup_n_value_deriv,
    lookup_apply_param,
    lookup_map_param,
    lookup_get_param_value,
    nullptr, nullptr,
    lookup_encode_param,
    lookup_read,
    lookup_write,
...
disp_lookup_new_from_comp(const char *name, disp_t *comp) {
    ...
    d->disp.lookup.component = (struct lookup_comp *) emalloc(sizeof(struct lookup_comp));
...
lookup_copy(const disp_t *src)
{
    ...
    struct lookup_comp *newcomp = (struct lookup_comp *) emalloc(sz);
    int j;
...
lookup_read(lexer_t *l, disp_t *d_gen)
{
    ...
    d->component = (struct lookup_comp *) emalloc(sizeof(struct lookup_comp) * nb);
    struct lookup_comp *comp = d->component;
...
void disp_lookup_add_comp(struct disp_struct *d_gen, int index, struct disp_struct *comp, double p)
{
    struct disp_lookup *d = &d_gen->disp.lookup;
    int n = d->nb_comps;
    struct lookup_comp *new_comp = (struct lookup_comp *) malloc(sizeof(struct lookup_comp) * (n + 1));
