...
struct disp_list app_lib[1] = {{nullptr, nullptr}};
struct disp_list user_lib[1] = {{nullptr, nullptr}};
struct disp_list preset_lib[1] = {{nullptr, nullptr}};

static struct disp_node *new_disp_node(disp_t *d, const char *id)
{
    struct disp_node *n = (struct disp_node *) malloc(sizeof(struct disp_node));
    if (id) {
        n->id = str_new();
        str_copy_c(n->id, id);
    } else {
